...
ALLOW_REAL = False  # This repair distribution remains DEMO only.
...
_SCALP_CAMPAIGN_META = {}
_SCALP_REENTRY_LOCK = {}        # (symbol,side) -> unix time      # (symbol,side) -> last add metadata
...
def trading_allowed(info):
    ...
    if kind == "REAL" and ALLOW_REAL:
        return True, ""
    if kind == "REAL":
        return False, f"REAL trading is disabled on Bridge V{BRIDGE_VERSION}. This repair does not permit REAL execution."
    ...
    if bool(cfg.get('scalp_campaign_enabled', True)):
        for key, group in groups.items():
            ...
            if len(group) >= 2:
                ...
                giveback_pct = max(18.0, min(safe_float(cfg.get('scalp_basket_peak_giveback_pct'), 28.0), 65.0)) / 100.0
