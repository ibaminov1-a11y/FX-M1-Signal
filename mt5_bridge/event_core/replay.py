...
from .compute_core import make_compute
...
class ReplayBroker:
    ...
    def __init__(self,meta,config,balance,latency_ms=200,slippage_ticks=3):
        ...
        timeframes=('M1','M5','M15','H1') if config.timeframe=='M5' else (config.timeframe,CONTEXT[config.timeframe])
    ...
    def current_bar(self,s,tf):
        return Bar(**self.current[tf]) if tf in self.current else None
    def history_position(self,pid):return [copy.deepcopy(d) for d in self._deals if d['position_id']==pid]
...
def replay(csv_path,meta,cfg,capital,latency_ms=0):
    ...
    with tempfile.TemporaryDirectory() as temp:
        store=Store(Path(temp)/'state.db');e=Engine(b,store,lambda:b.now);e.config=cfg;e.strategy.config=cfg;e.compute=make_compute(cfg)
        ...
        scenario_stats={}
        for (body,) in store.db.execute("SELECT body FROM journal WHERE kind='SCENARIO_EVENT'"):
            event=json.loads(body);group=scenario_stats.setdefault(event['type'],{})
            group[event['status']]=group.get(event['status'],0)+1
        out=dict(scenario_events=scenario_stats,ticks=seen,net_realized=b.balance-capital,open_floating=b.account()['equity']-b.balance,
