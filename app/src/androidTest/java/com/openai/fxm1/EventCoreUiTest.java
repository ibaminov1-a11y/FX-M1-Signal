...
public class EventCoreUiTest {
 ...
 @Test public void r3ChartRendersConfirmedSwingMapAndPath()throws Exception{
  JSONArray bars=new JSONArray()
    ...
    .put(new JSONObject().put("time",1800000600).put("open",1.1030).put("high",1.1035).put("low",1.1010).put("close",1.1015))
    .put(new JSONObject().put("time",1800000900).put("open",1.1015).put("high",1.1050).put("low",1.1012).put("close",1.1045));
  JSONArray structure=new JSONArray()
    .put(new JSONObject().put("kind","H").put("time",1800000000).put("price",1.1020))
    .put(new JSONObject().put("kind","HL").put("time",1800000600).put("price",1.1010))
    .put(new JSONObject().put("kind","HH").put("time",1800000900).put("price",1.1050));
  JSONArray levels=new JSONArray().put(new JSONObject().put("kind","trigger").put("time",1800000900).put("price",1.1040));
  final Bitmap[] bitmap={null};
  main(()->{SparklineView c=new SparklineView(rule.getActivity());c.layout(0,0,800,500);
   c.setMarket(bars,levels,new JSONArray(),structure,"IMPULSE");
   bitmap[0]=Bitmap.createBitmap(800,500,Bitmap.Config.ARGB_8888);c.draw(new Canvas(bitmap[0]));});
  ...
  assertTrue("R3 swing path must be visible",violet>40);
